...
class VolumeWindow(Gtk.Window):
    ...
    def crear_stack_de_vistas(self):
        ...
        for titulo, index in columnas:
            celda = Gtk.CellRendererText()
            columna = Gtk.TreeViewColumn(titulo, celda, text=index)
            columna.set_resizable(True)
            columna.connect("clicked", self.ordenar_por_columna, index, titulo.lower())
        ...
        # no cambiamos aún la vista visual, solo almacenamos el valor
        self.vista_predeterminada = vista_predeterminada

        scroll_tabla = Gtk.ScrolledWindow()
        
        scroll_tabla.set_policy(Gtk.PolicyType.AUTOMATIC, Gtk.PolicyType.AUTOMATIC)
